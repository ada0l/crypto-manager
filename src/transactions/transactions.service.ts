...
import { GeneralInfo, Transaction } from 'src/interfaces';
...
export class TransactionsService {
  ...
  public async create(transaction: Transaction): Promise<number> {
    return await this.createFew([transaction]).then((data) => data.at(0));
  }

  public async createFew(transactions: Transaction[]): Promise<number[]> {
    let result: number[];
  ...
  public async deleteById(transactionId: number): Promise<void> {
